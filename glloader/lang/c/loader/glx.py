...
int afwglLoadGLX(Display *dpy, int screen) {
	int status = 0;

	if(open_gl()) {
		status = afwglLoadGLXLoader((AFWGLloadproc)get_proc, dpy, screen);
		close_gl();
	}
...
_WGL_HEADER_MID = '''
#ifndef __afw_gl_glxext_h_

#ifdef __glxext_h_
#error GLX header already included, remove this include, aufw_glloader already provides it
#endif

#define __afw_gl_glxext_h_
#define __glxext_h_
...
_GLX_HEADER_LOADER = '''
GLAPI int afwglLoadGLX(Display *dpy, int screen);
''' + LOAD_OPENGL_DLL_H
...
_GLX_HAS_EXT = '''
static Display *AFWGLGLXDisplay = 0;
static int AFWGLGLXscreen = 0;
...
static int has_ext(const char *ext) {
	...
	if(!AFW_GL_GLX_VERSION_1_1)
		return 0;

	extensions = glXQueryExtensionsString(AFWGLGLXDisplay, AFWGLGLXscreen);
