...
from glloader.lang.common.generator import Generator
from glloader.lang.common.util import makefiledir
...
class CGenerator(Generator):
	...
	def open(self):
		...
		if self.local_files:
			self.h_include = '"Loader{}.h"'.format(suffix)
			self._f_c = open(make_path(self.path,
										'GLLoader{}.c'.format(suffix)), 'w')
			self._f_h = open(make_path(self.path, 
										'GLLoader{}.h'.format(suffix)), 'w')
			khr = self.path
		else:
			self.h_include = '<AuroraFW/GEngine/GL/Loader{}.h>'.format(suffix)
			self._f_c = open(make_path(self.path, 'src',
										'GLLoader{}.c'.format(suffix)), 'w')
			self._f_h = open(make_path(self.path, 'include', 'AuroraFW', 'GEngine', 'GL',
										'Loader{}.h'.format(suffix)), 'w')
			khr = os.path.join(self.path, 'include', 'AuroraFW', 'GEngine', 'GL', 'KHR')
	...
	def generate_loader(self, features, extensions):
		...
		for api, version in self.api.items():
			for feature in features[api]:
				f.write('static void load_{}(AFWGLloadproc load) {{\n'
						.format(feature.name))
				if self.spec.NAME in ('gl', 'glx', 'wgl'):
					f.write('\tif(!AFW_GL_{}) return;\n'.format(feature.name))
				for func in feature.functions:
					f.write('\tafw_gl_{0} = (PFN{1}PROC)load("{0}");\n'
			...
			if self.spec.NAME in ('gl', 'glx', 'wgl'):
				for feature in features[api]:
					f.write('\tAFW_GL_{} = (major == {num[0]} && minor >= {num[1]}) ||'
			...
			if api == 'glx':
				f.write('int afwglLoad{}Loader(AFWGLloadproc load, Display *dpy, int screen) {{\n'.format(api.upper()))
			elif api == 'wgl':
				f.write('int afwglLoad{}Loader(AFWGLloadproc load, HDC hdc) {{\n'.format(api.upper()))
			else:
				f.write('int afwglLoad{}Loader(AFWGLloadproc load) {{\n'.format(api.upper()))
	...
	def generate_types(self, types):
		...
		for type in types:
			...
			if output_string == '#include <AuroraFW/GEngine/GL/KHR/khrplatform.h>\n':
	...
	def write_api_header(self, f):
		for api in self.api:
			if api == 'glx':
				f.write('GLAPI int afwglLoad{}Loader(AFWGLloadproc, Display *dpy, int screen);\n\n'.format(api.upper()))
			elif api == 'wgl':
				f.write('GLAPI int afwglLoad{}Loader(AFWGLloadproc, HDC hdc);\n\n'.format(api.upper()))
			else:
				f.write('GLAPI int afwglLoad{}Loader(AFWGLloadproc);\n\n'.format(api.upper()))
